...
def dashboard(request, project_id):
    ...
    return render(request, 'user/manage/dashboard.html', context)
...
def issues_chart(request, project_id):
    ...
    result = models.Issues.objects.filter(project_id=project_id,
                                            create_datetime__gte=thirty_days_ago).extra(
            select={'ctime': "DATE_FORMAT(user_issues.create_datetime, '%%Y-%%m-%%d')"}).values('ctime').annotate(ct=Count('id'))
