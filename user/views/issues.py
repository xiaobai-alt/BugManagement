...
from user.forms.issues import IssuesModelForm, IssuesReplyModelForm, InviteModelForm
...
def issues_detail(request, project_id, issues_id):
    ...
    return render(request, 'user/manage/issues_detail.html', {'form': form, "issues_object": issues_object})
...
def issues_record(request, project_id, issues_id):
    ...
    # 判断是否可以评论和是否可以操作这个问题
    if request.method == "GET":
        reply_list = models.IssuesReply.objects.filter(issues_id=issues_id, issues__project=request.bug_management.project)
        # 将queryset转换为json格式
        data_list = []
...
def issues_change(request, project_id, issues_id):
    ...
    def create_reply_record(content):
        new_object = models.IssuesReply.objects.create(
            ...
            creator=request.bug_management.user,
        )
        new_reply_dict = {
            'id': new_object.id,
            'reply_type_text': new_object.get_reply_type_display(),
            'content': new_object.content,
            'creator': new_object.creator.user_name,
    ...
    if name in ['issues_type', 'module', 'parent', 'assign']:
        ...
        if not value:
            ...
            if name == 'assign':
                # 是否是项目创建者
                if value == str(request.bug_management.project.creator_id):
                    instance = request.bug_management.project.creator
                else:
    ...
    if name == "attention":
        ...
        if not value:
            ...
            # 获取当前项目的所有成员
            user_dict = {str(request.bug_management.project.creator_id): request.bug_management.project.creator.user_name}
            project_user_list = models.ProjectUser.objects.filter(project_id=project_id)
            for item in project_user_list:
                user_dict[str(item.user_id)] = item.user.user_name
            user_name_list = []
            ...
            issues_object.attention.set(value)
            issues_object.save()
            change_record = "{}更新为{}".format(field_object.verbose_name, ",".join(user_name_list))
        return JsonResponse({'status': True, 'data': create_reply_record(change_record)})
...
def invite_url(request, project_id):
    ...
    if form.is_valid():
        ...
        3. 限制：只有创建者才能邀请
        """
        if request.bug_management.user != request.bug_management.project.creator:
...
def invite_join(request, code):
    ...
    if not invite_object:
        return render(request, 'user/manage/invite_join.html', {'error': '邀请码不存在'})

    if invite_object.project.creator == request.bug_management.user:
        return render(request, 'user/manage/invite_join.html', {'error': '创建者无需再加入项目'})

    exists = models.ProjectUser.objects.filter(project=invite_object.project, user=request.bug_management.user).exists()
    if exists:
        return render(request, 'user/manage/invite_join.html', {'error': '已加入项目无需再加入'})

    # 最多允许的成员
    # max_member = request.bug_management.price_policy.project_member
    ...
    if current_member >= max_member:
        return render(request, 'user/manage/invite_join.html', {'error': '项目成员超限，请升级套餐'})
    ...
    if current_datetime > limit_datetime:
        return render(request, 'user/manage/invite_join.html', {'error': '邀请码已过期'})

    # 数量限制？
    if invite_object.count:
        if invite_object.use_count >= invite_object.count:
            return render(request, 'user/manage/invite_join.html', {'error': '邀请码数据已使用完'})
        invite_object.use_count += 1
        invite_object.save()

    models.ProjectUser.objects.create(user=request.bug_management.user, project=invite_object.project)

    # ####### 问题2： 更新项目参与成员 #######
    invite_object.project.join_count += 1
    invite_object.project.save()

    return render(request, 'user/manage/invite_join.html', {'project': invite_object.project})
